...
-- ITEMS
-- seller_id must match existing users.user_id (1..6 from above)
...
-- AUCTIONS
-- Auctions 1 & 3: FORWARD
-- Auctions 2,4,5,6: DUTCH
...
VALUES 
-- Forward Auctions with short timers
(1, 1, NULL, 'FORWARD', 100, 120, NOW() + INTERVAL '3 minute', 10, 15, 5),  -- auction_id = 1
(3, 1, NULL, 'FORWARD', 600, 650, NOW() + INTERVAL '3 minute', 12, 18, 5),  -- auction_id = 2

-- Dutch Auctions with longer timers
(2, 4, NULL, 'DUTCH',  500,  500, NOW() + INTERVAL '10 minutes', 15, 20, 7), -- auction_id = 3
(4, 5, NULL, 'DUTCH', 2000, 2000, NOW() + INTERVAL '15 minutes', 20, 30,10), -- auction_id = 4
(5, 4, NULL, 'DUTCH',  800,  800, NOW() + INTERVAL '20 minutes', 15, 22, 7), -- auction_id = 5
(6, 5, NULL, 'DUTCH',  600,  600, NOW() + INTERVAL '30 minutes', 10, 15, 5); -- auction_id = 6

-- FORWARD_AUCTIONS
-- These reference auction_ids for FORWARD auctions: 1 and 2
INSERT INTO forward_auctions (auction_id, min_increment, reserve_price) VALUES
(1, 10, 100),  -- Vintage Clock: min increment $10, reserve $100
(2, 25, 600);  -- Laptop: min increment $25, reserve $600

-- DUTCH_AUCTIONS
-- These reference auction_ids for DUTCH auctions: 3,4,5,6
-- reserve_price = bottom price (clamp floor for dynamic price calculation)
INSERT INTO dutch_auctions (auction_id, price_drop_step, step_interval_sec, reserve_price) VALUES
(3, 25,  60,  400),  -- Art Painting: drops $25 every 60s, never below $400
(4, 100, 300, 1500), -- Designer Watch: drops $100 every 5 min, floor $1500
(5, 50,  120, 700),  -- Vintage Camera: drops $50 every 2 min, floor $700
(6, 30,  180, 500);  -- Gaming Console: drops $30 every 3 min, floor $500

-- BIDS (FORWARD auctions only: auction_id 1 and 2)
INSERT INTO bids (auction_id, bidder_id, amount) VALUES
(1, 2, 110),  -- Bob bids $110 on Vintage Clock
(1, 3, 120),  -- Charlie bids $120 on Vintage Clock
(2, 2, 625),  -- Bob bids $625 on Laptop
(2, 3, 650);  -- Charlie bids $650 on Laptop

-- DUTCH_ACCEPTS (Dutch auctions: 3,4,5,6)
INSERT INTO dutch_accepts (auction_id, buyer_id, accepted_price, accepted) VALUES
-- Art Painting (auction_id: 3)
(3, 3, 500, true),   -- Charlie accepted at starting price
(3, 2, 475, false),  -- Bob rejected at $475
...
(1, 3, '789 Elm St, Montreal, QC, H3B 1A1', false, 'COMPLETED'), -- Charlie paid for Vintage Clock
(3, 3, '789 Elm St, Montreal, QC, H3B 1A1', true,  'COMPLETED'), -- Charlie paid for Art Painting
(2, 2, '456 Oak Ave, Vancouver, BC, V6B 1A1', false, 'PENDING'),  -- Bob pending on Laptop
(4, 3, '789 Elm St, Montreal, QC, H3B 1A1', true,  'COMPLETED'), -- Charlie paid for Designer Watch
(5, 6, '987 Cedar Ln, Halifax, NS, B3H 1A1', false, 'PENDING'),  -- Frank pending on Vintage Camera
(6, 6, '987 Cedar Ln, Halifax, NS, B3H 1A1', true,  'PENDING');  -- Frank pending on Gaming Console
...
SELECT a.auction_id, i.title, u.username AS seller, a.auction_type,
       a.start_price, a.current_price, a.end_time,
       a.shipping_price, a.expedited_price, a.winner_id
FROM auctions a
...
SELECT p.payment_id, u.username AS buyer, i.title AS item,
       p.shipping_address, p.expedited, p.payment_status, p.created_at
FROM payments p
